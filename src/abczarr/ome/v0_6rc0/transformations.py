...
class Space(OMEMetadata):
    ...
    [CoordinateSystem][abczarr.ome.v0_6rc0.systems.CoordinateSystem] that a
...
class CoordinateTransformation(OMEMetadata):
    ...
    subclass, such as [Scale][abczarr.ome.v0_6rc0.transformations.Scale].
    `input` and `output` are each a
    [Space][abczarr.ome.v0_6rc0.transformations.Space] that identifies the
...
class ProjectAxis(CoordinateTransformation):
    """A `ProjectAxis` transformation changes the number of axes between input
    and output.
    ...
    droppedInputs: Optional[tx.List[int]]
