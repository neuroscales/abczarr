...
[OME][abczarr.ome.v0_6dev2.ome.OME] and its subclasses each describe what
...
class OMEImage(OME):
    ...
    """The group's
    [Multiscale][abczarr.ome.v0_6dev2.images.Multiscale] pyramids."""
...
class OMEImageLabel(OMEImage):
    ...
    Extends [OMEImage][abczarr.ome.v0_6dev2.ome.OMEImage] with the
    metadata describing the segments the image's pixel values name.
    """

    image_label: Required[ImageLabel] = field(json="image-label")
    """The [ImageLabel][abczarr.ome.v0_6dev2.labels.ImageLabel] metadata
    for the segments."""
...
class OMEPlate(OME):
    ...
    """The [Plate][abczarr.ome.v0_6dev2.plates.Plate] metadata describing
...
class OMEWell(OME):
    ...
    well: Required[Well]
    """The [Well][abczarr.ome.v0_6dev2.wells.Well] metadata listing the
...
class OMEBioformats2Raw(OME):
    ...
    plate: Required[Plate]
    """The [Plate][abczarr.ome.v0_6dev2.plates.Plate] metadata for the
