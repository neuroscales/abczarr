...
class OME(OMEBase):
    ...
    what it holds. See [OMEImage][abczarr.ome.v0_1.ome.OMEImage],
    [OMEPlate][abczarr.ome.v0_1.ome.OMEPlate], and
    [OMEWell][abczarr.ome.v0_1.ome.OMEWell] for examples. The
...
class OMEImage(OME):
    ...
    [Multiscale][abczarr.ome.v0_1.images.Multiscale] object. An
    image almost always has exactly one. The optional `omero` field
    suggests how a viewer should display the image's channels. See
    [Omero][abczarr.ome.v0_1.omero.Omero] for the fields it carries.
    """
...
class OMEImageLabel(OMEImage):
    ...
    A label image is an [OMEImage][abczarr.ome.v0_1.ome.OMEImage]
    whose pixel values name segments. The `image_label` field
    carries the [ImageLabel][abczarr.ome.v0_1.labels.ImageLabel]
    metadata that describes those segments, including their display
    colors and any per-label properties.
    """

    image_label: Required[tx.List[ImageLabel]] = field(json="image-label")
...
class OMELabels(OME):
    ...
    [OMEImageLabel][abczarr.ome.v0_1.ome.OMEImageLabel] object.
    """
...
class OMEPlate(OME):
    ...
    [Plate][abczarr.ome.v0_1.plates.Plate] metadata, which describes
...
class OMEWell(OME):
    ...
    [Well][abczarr.ome.v0_1.wells.Well] metadata, which describes
    the group's fields of view. For each field of view, that
    metadata also names the acquisition run it belongs to.
    """
...
class OMEBioformats2Raw(OME):
    """Metadata for the root group of a bioformats2raw-converted
    dataset.
    ...
    the [Plate][abczarr.ome.v0_1.plates.Plate] metadata for that
    plate.
    """
