...
[OME][abczarr.ome.v0_3.ome.OME] and its subclasses each describe what
...
class OME(OMEBase):
    ...
    [OMEImage][abczarr.ome.v0_3.ome.OMEImage],
    [OMEPlate][abczarr.ome.v0_3.ome.OMEPlate], and
    [OMEWell][abczarr.ome.v0_3.ome.OMEWell] for examples.
...
class OMEImageLabel(OMEImage):
    ...
    A label image is an [OMEImage][abczarr.ome.v0_3.ome.OMEImage] whose
    pixel values name segments.

    Parameters
    ----------
    image_label : ImageLabel
        The [ImageLabel][abczarr.ome.v0_3.labels.ImageLabel] metadata
...
class OMEPlate(OME):
    ...
    ----------
    plate : Plate
        The [Plate][abczarr.ome.v0_3.plates.Plate] metadata describing
...
class OMEWell(OME):
    ...
    ----------
    well : Well
        The [Well][abczarr.ome.v0_3.wells.Well] metadata describing the
        group's fields of view. For each field of view, that metadata
        also names the acquisition run it belongs to.
    """
...
class OMEBioformats2Raw(OME):
    ...
    bioformats2raw_layout : int
        ...
        value is always `3`.
    plate : Plate
        The [Plate][abczarr.ome.v0_3.plates.Plate] metadata, present when
